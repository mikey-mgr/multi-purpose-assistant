"""
Prefect flows for the automated job application pipeline.

Exposes four independently-deployable flows:
  1. scrape-and-store      — run scrapers, insert jobs
  2. match-jobs            — batch-classify unscored jobs
  3. generate-matched      — generate docs for matched jobs
  4. apply-agent           — process apply_instructions for matched jobs

When ``scrape-and-store`` runs via schedule, it auto-chains 02→03→04.
Manual runs stop at scrape.
"""

import logging

from prefect import flow, task, get_run_logger

from app.config import settings
from app.matcher import batch_match_jobs
from app.orchestrator import process_job_for_user, batch_process_applications
from app.whatsapp_notifier import send_flow_error_notification
from core.database import (
    get_matched_unprocessed_jobs,
    update_job_match_status,
    normalize_job_title,
    get_session,
    ScrapedJob,
    JobMatch,
    count_unscored_jobs,
    count_matched_unprocessed_jobs,
    count_generated_unapplied_jobs,
)
from scrapers.unified_scraper import UnifiedJobScraper

logger = logging.getLogger(__name__)


# ── Tasks ──────────────────────────────────────────────────────────────

@task(retries=2, retry_delay_seconds=30)
def scrape_jobs_task(
    site_names: list[str] | None = None,
    max_pages: dict = {},
) -> "pd.DataFrame | None":
    """Scrape job boards and return DataFrame. Insert happens in a separate task."""
    import pandas as pd  # noqa: F401 — keep for type hint
    run_logger = get_run_logger()
    site_names = site_names or ["iharare", "vacancybox", "vacancymail"]
    run_logger.info("Scraping job boards %s (max_pages=%s) ...", site_names, max_pages)
    scraper = UnifiedJobScraper()
    kwargs = {"max_pages": max_pages} if max_pages else {}
    jobs_df = scraper.scrape_jobs(
        site_name=site_names,
        **kwargs,
    )
    if jobs_df.empty:
        run_logger.warning("No jobs found from any site.")
        return None
    run_logger.info("Scraped %d jobs.", len(jobs_df))
    return jobs_df


@task(retries=3, retry_delay_seconds=15)
def store_jobs_task(jobs_df: "pd.DataFrame") -> int:
    """Insert scraped jobs into PostgreSQL. Retries independently of scraping."""
    from core.database import init_db, insert_jobs
    run_logger = get_run_logger()
    init_db()
    count = insert_jobs(jobs_df.to_dict("records"), log_fn=run_logger.info)
    run_logger.info("Inserted %d new jobs into database (of %d scraped).", count, len(jobs_df))
    return count


@task(retries=2, retry_delay_seconds=30)
def match_pending_jobs(
    user_id: str,
    limit: int = 50,
    match_model: str = "models/gemini-3.5-flash-lite",
    match_provider: str = "gemini",
    match_fallback_model: str = "nvidia/nemotron-3-ultra-550b-a55b:free",
    match_fallback_provider: str = "openrouter",
) -> int:
    """Batch-classify unscored jobs and persist decisions."""
    decisions = batch_match_jobs(
        user_id, limit=limit, model=match_model, provider=match_provider,
        fallback_model=match_fallback_model, fallback_provider=match_fallback_provider,
    )
    return len(decisions)


@task(retries=2, retry_delay_seconds=30)
def fetch_matched_jobs(user_id: str, limit: int = 10) -> list:
    """Fetch matched jobs that haven't been generated yet."""
    return get_matched_unprocessed_jobs(user_id, limit=limit)


@task
def generate_application(
    job_id: int,
    user_id: str,
    generate_model: str | None = None,
    generate_provider: str | None = None,
    generate_fallback_model: str | None = None,
    generate_fallback_provider: str | None = None,
) -> dict:
    """Run the full generation pipeline for one matched job + user."""
    docs = process_job_for_user(
        user_id=user_id,
        job_id=job_id,
        model=generate_model,
        provider=generate_provider,
        fallback_model=generate_fallback_model,
        fallback_provider=generate_fallback_provider,
    )
    if docs:
        update_job_match_status(job_id, user_id, "generated")
    return {
        "job_id": job_id,
        "user_id": user_id,
        "documents": len(docs),
        "status": "completed" if docs else "failed",
    }


@task
def dedup_generated_jobs(user_id: str) -> int:
    """Post-generation dedup sweep: for duplicate (title, company) groups,
    keep the first generated job and mark the rest as 'duplicate'."""
    from collections import defaultdict
    run_logger = get_run_logger()
    session = get_session()
    try:
        rows = session.query(JobMatch, ScrapedJob).join(
            ScrapedJob, JobMatch.job_id == ScrapedJob.id
        ).filter(
            JobMatch.user_id == user_id,
            JobMatch.status == "generated",
        ).all()

        groups: dict[tuple[str, str], list[tuple[JobMatch, ScrapedJob]]] = defaultdict(list)
        for match, job in rows:
            key = (normalize_job_title(job.title or ""), (job.company or "").strip().lower())
            groups[key].append((match, job))

        dedup_count = 0
        for key, group in groups.items():
            if len(group) <= 1:
                continue
            # Keep first (lowest job_id), mark rest as duplicate
            group.sort(key=lambda x: x[0].job_id)
            for match, _ in group[1:]:
                match.status = "duplicate"
                session.commit()
                dedup_count += 1
                run_logger.info(
                    "Dedup: marked JobMatch #%d (job #%d) as duplicate (same title+company as #%d)",
                    match.id, match.job_id, group[0][0].job_id,
                )

        return dedup_count
    finally:
        session.close()


@task
def mark_processed(result: dict) -> None:
    """Log result."""
    get_run_logger().info(
        "Job %s processed for user %s — %d documents generated.",
        result["job_id"],
        result["user_id"],
        result["documents"],
    )


# ── Flows ──────────────────────────────────────────────────────────────

@flow(
    name="scrape-and-store",
    description="Scrape all Zimbabwe job boards and insert new postings.",
    retries=1,
    retry_delay_seconds=60,
    log_prints=True,
)
def scrape_and_store(
    site_names: list[str] | None = None,
    max_pages: dict = {},
    user_id: str | None = None,
    match_model: str | None = None,
    match_provider: str | None = None,
    match_fallback_model: str | None = None,
    match_fallback_provider: str | None = None,
    generate_model: str | None = None,
    generate_provider: str | None = None,
    generate_fallback_model: str | None = None,
    generate_fallback_provider: str | None = None,
    match_limit: int = 50,
    job_limit: int = 10,
    chain_next: bool = False,
    max_iterations: int = 3,
):
    """Scrape job boards. If chain_next=True, triggers 02→03→04 after scraping."""
    run_logger = get_run_logger()
    try:
        jobs_df = scrape_jobs_task(site_names=site_names, max_pages=max_pages)
        if jobs_df is None:
            run_logger.info("No jobs scraped.")
            return
        count = store_jobs_task(jobs_df)
        run_logger.info("Scrape complete — %d new jobs inserted.", count)

        if not chain_next:
            run_logger.info("chain_next=False — stopping after scrape.")
            return

        if not user_id:
            run_logger.warning("No user_id — skipping downstream pipeline.")
            return

        run_logger.info("chain_next=True — chaining 02→03→04...")

        # Step 2 — match unscored jobs
        matched_count = match_jobs_flow(
            user_id=user_id,
            limit=match_limit,
            match_model=match_model,
            match_provider=match_provider,
            match_fallback_model=match_fallback_model,
            match_fallback_provider=match_fallback_provider,
            chain_next=True,
            max_iterations=max_iterations,
        )

        if matched_count == 0:
            run_logger.info("No matches found — 03 and 04 skipped.")
            return

        # Steps 3 and 4 are triggered by match_jobs_flow when chain_next=True
        run_logger.info("Downstream pipeline complete.")
    except Exception as e:
        run_logger.error("Scrape-and-store flow failed: %s", e)
        send_flow_error_notification("scrape-and-store", str(e))
        raise


@flow(
    name="match-jobs",
    description="Batch-classify unscored jobs via LLM.",
    retries=1,
    retry_delay_seconds=60,
    log_prints=True,
)
def match_jobs_flow(
    user_id: str,
    limit: int = 50,
    match_model: str = "models/gemini-3.5-flash-lite",
    match_provider: str = "gemini",
    match_fallback_model: str = "nvidia/nemotron-3-ultra-550b-a55b:free",
    match_fallback_provider: str = "openrouter",
    chain_next: bool = False,
    max_iterations: int = 3,
) -> int:
    """Match unscored jobs for a user. Loops until all caught up or max_iterations. If chain_next=True, triggers 03→04."""
    run_logger = get_run_logger()
    try:
        total_matched = 0
        for iteration in range(1, max_iterations + 1):
            remaining = count_unscored_jobs(user_id)
            if remaining == 0:
                break
            run_logger.info("Iteration %d/%d: %d unscored jobs remaining.", iteration, max_iterations, remaining)
            matched = match_pending_jobs(
                user_id, limit=limit, match_model=match_model, match_provider=match_provider,
                match_fallback_model=match_fallback_model, match_fallback_provider=match_fallback_provider,
            )
            total_matched += matched

        run_logger.info("Match complete — %d total decisions saved.", total_matched)

        if chain_next and total_matched > 0:
            run_logger.info("chain_next=True — triggering 03-generator...")
            generate_matched_flow(
                user_id=user_id,
                limit=limit,
                generate_model=None,
                generate_provider=None,
                generate_fallback_model=None,
                generate_fallback_provider=None,
                chain_next=True,
                max_iterations=max_iterations,
            )

        return total_matched
    except Exception as e:
        run_logger.error("Match flow failed: %s", e)
        send_flow_error_notification("match-jobs", str(e))
        raise


@flow(
    name="generate-matched",
    description="Generate resume + cover letter for all matched-but-unprocessed jobs.",
    retries=1,
    retry_delay_seconds=60,
    log_prints=True,
)
def generate_matched_flow(
    user_id: str,
    limit: int = 10,
    generate_model: str | None = None,
    generate_provider: str | None = None,
    generate_fallback_model: str | None = None,
    generate_fallback_provider: str | None = None,
    chain_next: bool = False,
    max_iterations: int = 3,
):
    """
    Generate documents for matched jobs. Loops until all caught up or max_iterations. If chain_next=True, triggers 04-apply-agent.
    """
    run_logger = get_run_logger()
    try:
        total_generated = 0
        for iteration in range(1, max_iterations + 1):
            remaining = count_matched_unprocessed_jobs(user_id)
            if remaining == 0:
                break
            run_logger.info("Iteration %d/%d: %d matched-but-unprocessed jobs remaining.", iteration, max_iterations, remaining)
            jobs = fetch_matched_jobs(user_id, limit=limit)
            if not jobs:
                break

            results = generate_application.map(
                job_id=[j.id for j in jobs],
                user_id=[user_id] * len(jobs),
                generate_model=[generate_model] * len(jobs),
                generate_provider=[generate_provider] * len(jobs),
                generate_fallback_model=[generate_fallback_model] * len(jobs),
                generate_fallback_provider=[generate_fallback_provider] * len(jobs),
            )

            for r in results:
                mark_processed(r)
            total_generated += len(jobs)

        if total_generated:
            dedup_count = dedup_generated_jobs(user_id)
            if dedup_count:
                run_logger.info("Post-generation dedup: %d duplicate jobs marked.", dedup_count)

        if chain_next and total_generated > 0:
            run_logger.info("chain_next=True — triggering 04-apply-agent...")
            apply_agent_flow(
                user_id=user_id,
                limit=limit,
                generate_model=generate_model,
                generate_provider=generate_provider,
                max_iterations=max_iterations,
            )
    except Exception as e:
        run_logger.error("Generate flow failed: %s", e)
        send_flow_error_notification("generate-matched", str(e))
        raise


@flow(
    name="apply-agent",
    description="Parse apply_instructions for matched jobs and send email / WhatsApp notification.",
    retries=1,
    retry_delay_seconds=60,
    log_prints=True,
)
def apply_agent_flow(
    user_id: str,
    limit: int = 10,
    generate_model: str | None = None,
    generate_provider: str | None = None,
    chain_next: bool = False,
    max_iterations: int = 3,
):
    """
    Process apply_instructions for matched-but-unapplied jobs. Loops until all caught up or max_iterations.

    chain_next is accepted for pipeline consistency (no next flow to chain to).
    """
    run_logger = get_run_logger()
    try:
        total_processed = 0
        for iteration in range(1, max_iterations + 1):
            remaining = count_generated_unapplied_jobs(user_id)
            if remaining == 0:
                break
            run_logger.info("Iteration %d/%d: %d unapplied jobs remaining.", iteration, max_iterations, remaining)
            results = batch_process_applications(
                user_id=user_id,
                limit=limit,
                model=generate_model,
                provider=generate_provider,
            )
            total_processed += len(results)
            for r in results:
                run_logger.info(
                    "Apply agent: job=%s action=%s email=%s whatsapp=%s",
                    r.get("job_id"), r.get("action"),
                    r.get("email_sent"), r.get("whatsapp_sent"),
                )
        run_logger.info("Batch apply complete — %d jobs processed across %d iterations.", total_processed, iteration)
    except Exception as e:
        run_logger.error("Apply agent flow failed: %s", e)
        send_flow_error_notification("apply-agent", str(e))
        raise


@flow(
    name="manual-generate",
    description="One-shot generation for a specific job + user.",
)
def manual_generate(
    user_id: str,
    job_id: int,
    generate_model: str | None = None,
    generate_provider: str | None = None,
):
    """Trigger document generation for one specific job posting."""
    result = generate_application(
        job_id=job_id,
        user_id=user_id,
        generate_model=generate_model,
        generate_provider=generate_provider,
    )
    mark_processed(result)
    return result


# ── CLI entry point ────────────────────────────────────────────────────

if __name__ == "__main__":
    import sys
    if len(sys.argv) >= 3 and sys.argv[1] == "--manual":
        manual_generate(user_id=sys.argv[2], job_id=int(sys.argv[3]))
    else:
        user_id = sys.argv[1] if len(sys.argv) >= 2 else "default"
        scrape_and_store(user_id=user_id)
